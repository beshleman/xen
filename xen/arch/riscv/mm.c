...
/* Allocate a new page, format it as a valid PTE,
 * point *entry to the valid PTE, and return;
 */
                          ...
                          unsigned long linker_addr_end)
{
     ...
     *
     * TODO: Protect text and rodata from writes.
     */
...
void __init setup_frametable_mappings(paddr_t ps, paddr_t pe)
{
    ...
    for ( i = ROUNDUP(FRAMETABLE_VIRT_START, MB(2)); i < virt_end; i += MB(2) )
    {
        ...
        /* Convert the machine frame to be mapped in to a PTE */
        pte = mfn_to_xen_entry(mfn);
        pte.pte |= PTE_LEAF_DEFAULT;

        /* Point the first-level table to the machine frame by loading the pte into the table */
        ...
        /* Point the second-level table to the first-level table by loading the pte into the second table */
